...
class GameScene: SKScene {
    ...
    let TileWidth: CGFloat = UIDevice.currentDevice().userInterfaceIdiom == .Pad ? 64 : 32.0
	let TileHeight: CGFloat = UIDevice.currentDevice().userInterfaceIdiom == .Pad ? 72 : 36.0
    ...
    func showSelectionIndicatorForCookie(cookie: Cookie) {
        ...
        if let sprite = cookie.sprite {
            let texture = SKTexture(imageNamed: cookie.cookieType.spriteName)
            selectionSprite.size = texture.size()
            selectionSprite.runAction(SKAction.setTexture(texture))
            
            var systemVersion = UIDevice.currentDevice().systemVersion
			selectionSprite.size = CGSizeMake(texture.size().width / UIScreen.mainScreen().scale, texture.size().height / UIScreen.mainScreen().scale)
			//Fix Apple bug in iOS version < 8, see http://stackoverflow.com/questions/20969392/sktexture-and-the-scale-property-of-a-uiimage
			if systemVersion.compare("8.0", options: NSStringCompareOptions.NumericSearch, range: nil, locale: nil) == NSComparisonResult.OrderedAscending
			{
